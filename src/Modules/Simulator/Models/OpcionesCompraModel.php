...
/**
 * Model for Opciones de Compra (Bienes Muebles) CRUD operations.
 * Wraps the StudentAttemptModel for loading/saving borrador_json.
 */
class OpcionesCompraModel
{
    ...
    public function getIntento(int $intentoId): ?array
    {
        try {
            ...
            error_log('[OpcionesCompraModel::getIntento] ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getIntentoActivo(): ?array
    {
        try {
            ...
            error_log('[OpcionesCompraModel::getIntentoActivo] ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function guardarBorrador(int $intentoId, array $borrador, int $pasoActual, string $pasosCompletados = ''): bool
    {
        try {
            ...
            error_log('[OpcionesCompraModel::guardarBorrador] ' . $e->getMessage());
            throw $e;
        }
    }
}
