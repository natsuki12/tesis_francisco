...
/**
 * Model for Tarjetas de Crédito (Pasivos Deuda) CRUD operations.
 * Wraps the StudentAttemptModel for loading/saving borrador_json.
 */
class TarjetasCreditoModel
{
    ...
    public function getIntento(int $intentoId): ?array
    {
        try {
            ...
            error_log('[TarjetasCreditoModel::getIntento] ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getIntentoActivo(): ?array
    {
        try {
            ...
            error_log('[TarjetasCreditoModel::getIntentoActivo] ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function guardarBorrador(int $intentoId, array $borrador, int $pasoActual, string $pasosCompletados = ''): bool
    {
        try {
            ...
            error_log('[TarjetasCreditoModel::guardarBorrador] ' . $e->getMessage());
            throw $e;
        }
    }
}
