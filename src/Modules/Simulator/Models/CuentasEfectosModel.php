...
/**
 * Model for Cuentas y Efectos por Cobrar (Bienes Muebles) CRUD operations.
 * Wraps the StudentAttemptModel for loading/saving borrador_json.
 */
class CuentasEfectosModel
{
    ...
    public function getIntento(int $intentoId): ?array
    {
        try {
            ...
            error_log('[CuentasEfectosModel::getIntento] ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getIntentoActivo(): ?array
    {
        try {
            ...
            error_log('[CuentasEfectosModel::getIntentoActivo] ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function guardarBorrador(int $intentoId, array $borrador, int $pasoActual, string $pasosCompletados = ''): bool
    {
        try {
            ...
            error_log('[CuentasEfectosModel::guardarBorrador] ' . $e->getMessage());
            throw $e;
        }
    }
}
