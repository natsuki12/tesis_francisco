...
/**
 * Model for Semovientes (Bienes Muebles) CRUD operations.
 * Wraps the StudentAttemptModel for loading/saving borrador_json.
 */
class SemovientesModel
{
    ...
    public function getIntento(int $intentoId): ?array
    {
        try {
            ...
            error_log('[SemovientesModel::getIntento] ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getIntentoActivo(): ?array
    {
        try {
            ...
            error_log('[SemovientesModel::getIntentoActivo] ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function guardarBorrador(int $intentoId, array $borrador, int $pasoActual, string $pasosCompletados = ''): bool
    {
        try {
            ...
            error_log('[SemovientesModel::guardarBorrador] ' . $e->getMessage());
            throw $e;
        }
    }
}
