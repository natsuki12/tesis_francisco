...
/**
 * Model for Crédito Hipotecario (Pasivos Deuda) CRUD operations.
 * Wraps the StudentAttemptModel for loading/saving borrador_json.
 */
class CreditoHipotecarioModel
{
    ...
    public function getIntento(int $intentoId): ?array
    {
        try {
            ...
            error_log('[CreditoHipotecarioModel::getIntento] ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function getIntentoActivo(): ?array
    {
        try {
            ...
            error_log('[CreditoHipotecarioModel::getIntentoActivo] ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function guardarBorrador(int $intentoId, array $borrador, int $pasoActual, string $pasosCompletados = ''): bool
    {
        try {
            ...
            error_log('[CreditoHipotecarioModel::guardarBorrador] ' . $e->getMessage());
            throw $e;
        }
    }
}
