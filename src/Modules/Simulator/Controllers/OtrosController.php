...
use App\Modules\Simulator\Models\OtrosModel;

/**
 * Controller for Otros (Bienes Muebles) view and CRUD API.
 * Has a unique "Tipo de Bien" select field (value=19 → "Otros Especifique").
 */
class OtrosController
{
    private OtrosModel $model;

    public function __construct()
    {
        $this->model = new OtrosModel();
    }
    ...
    public function index(object $app): string
    {
        ...
        return $app->view('simulator/seniat_actual/sucesion/bienes_muebles/otros', [
            'intento' => $intento,
        ]);
    }

    /** POST /api/otros/{intentoId}/agregar */
    public function agregar(int $intentoId): void
    {
        ...
        try {
            ...
            $items = $borrador['bienes_muebles_otros'] ?? [];

            $items[] = $this->buildOtro($input);

            $borrador['bienes_muebles_otros'] = $items;
            ...
            error_log('[Otros::agregar] ' . $e->getMessage());
            http_response_code(500);
            echo json_encode(['ok' => false, 'error' => 'Error interno del servidor']);
        }
    }

    /** POST /api/otros/{intentoId}/editar */
    public function editar(int $intentoId): void
    {
        ...
        try {
            ...
            $items[$idx] = $this->buildOtro($input, $items[$idx]);
            ...
            error_log('[Otros::editar] ' . $e->getMessage());
            http_response_code(500);
            echo json_encode(['ok' => false, 'error' => 'Error interno del servidor']);
        }
    }

    /** POST /api/otros/{intentoId}/eliminar */
    public function eliminar(int $intentoId): void
    {
        ...
        try {
            ...
            $borrador['bienes_muebles_otros'] = array_values($items);
            $ok = $this->model->guardarBorrador(
                $intentoId,
                $borrador,
                (int) ($intento['paso_actual'] ?? 1),
            ...
            error_log('[Otros::eliminar] ' . $e->getMessage());
            http_response_code(500);
            echo json_encode(['ok' => false, 'error' => 'Error interno del servidor']);
        }
    }
    ...
    /**
     * Build an "otros" record — includes cod_tipo_bien / nombre_tipo_bien.
     */
    private function buildOtro(array $input, array $existing = []): array
    {
        return [
            'cod_tipo_bien'    => $input['cod_tipo_bien'] ?? $existing['cod_tipo_bien'] ?? '19',
            'nombre_tipo_bien' => $input['nombre_tipo_bien'] ?? $existing['nombre_tipo_bien'] ?? 'Otros Especifique',
