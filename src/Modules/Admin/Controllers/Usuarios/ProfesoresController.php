    ...
    public function guardar()
    {
        ...
        try {
            ...
            try {
                ...
                error_log('[ProfesoresController::guardar] Advertencia: Fallo al enviar email asíncrono: ' . $m->getMessage());
            }

            // ── 8. Respuesta ──
            $message = $emailSent
                ? 'Profesor registrado exitosamente. Se ha enviado un correo de bienvenida.'
                : 'Profesor registrado exitosamente. El correo de bienvenida será entregado en breve (fallo temporal al despachar).';
    ...
    public function importarCSV()
    {
        ...
        try {
            ...
            while (($row = fgetcsv($handle, 1000, $delimiter)) !== false) {
                ...
                $email    = trim($row[0]);
                // Eliminación de BOM invisible \xEF\xBB\xBF
                $email    = preg_replace('/^\xEF\xBB\xBF/', '', $email);
                // Normalización obligatoria a minúsculas
                $email    = mb_strtolower($email, 'UTF-8');
                
                $cedulaRaw = strtoupper(trim($row[1]));
                // Limpieza de Cédula: remover guiones y espacios ("V- 123" => "V123")
                $cedulaRaw = preg_replace('/[\s\-\.,]/', '', $cedulaRaw);
                
                $nombres  = trim($row[2]);
                $apellidos = trim($row[3]);

                // ── Detección e Ignorado Silencioso de Cabeceras ──
                if ($lineNum === 1) {
                ...
                try {
                    ...
                    // Encolar correo de bienvenida (Proteger del rollback si hay fallo SMTP)
                    try {
                        $baseUrl = rtrim($_ENV['APP_BASE'] ?? 'http://localhost/tesis_francisco', '/');
                        $emailBody = $this->buildWelcomeEmail($nombres, $apellidos, $email, $cedula, $baseUrl);
                        ...
                        error_log("[ProfesoresController::importarCSV] Error encolando email fila $lineNum: " . $m->getMessage());
                        // Avisar al usuario visualmente sin distorsionar la estadística
                        $errors[] = "Fila $lineNum: El profesor fue creado existosamente, pero falló el encolamiento de su correo de bienvenida.";
                    }
    ...
    private function buildWelcomeEmail(
        ...
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;'>
            <div style='background: linear-gradient(135deg, #1a237e, #283593); color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;'>
                <img src='https://raw.githubusercontent.com/natsuki12/tesis_francisco/refs/heads/main/public/assets/img/logos/sucelab/logo_Mesa%20de%20trabajo%201-04.png' alt='SUCELAB Logo' style='display: block; margin: 0 auto 15px auto; max-width: 150px; height: auto;'>
