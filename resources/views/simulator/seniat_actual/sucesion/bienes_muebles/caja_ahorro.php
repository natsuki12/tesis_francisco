...
$cajaAhorroGuardadas = $borrador['bienes_muebles_caja_ahorro'] ?? [];

ob_start();
?>

<div _ngcontent-sdd-c96 class="shadow-lg p-3 mb-5 bg-body rounded lenletra">
    <div _ngcontent-sdd-c96 class=card>
        <div _ngcontent-sdd-c96 class=card-header>Caja de Ahorro</div>
        <div _ngcontent-sdd-c96 class=card-body>
            <form _ngcontent-sdd-c96 novalidate>

                <!-- ═══ Row 1: Rif Empresa · Razón Social · Bien Litigioso ═══ -->
                <div _ngcontent-sdd-c96 class=row>
                    <div _ngcontent-sdd-c96 class=col-sm-4>
                        <div _ngcontent-sdd-c96 class=form-group>
                            <div _ngcontent-sdd-c96 class="form-floating sm-3">
                                <input _ngcontent-sdd-c96 id=rifEmpresa placeholder=# type=text
                                    formcontrolname=rifEmpresa maxlength=10 required
                                    class="form-control form-control-sm" value="">
                                <label _ngcontent-sdd-c96 for=rifEmpresa>Rif Empresa</label>
                            </div>
                            <div _ngcontent-sdd-c96 id=errorRifEmpresa class="col-sm-4 text-danger" style="display:none"></div>
                        </div>
                    </div>
                    <div _ngcontent-sdd-c96 class=col-sm-6>
                        <div _ngcontent-sdd-c96 class=form-group>
                            <div _ngcontent-sdd-c96 class=form-floating>
                                <input _ngcontent-sdd-c96 id=razonSocial placeholder=# type=text
                                    formcontrolname=razonSocial required
                                    class="form-control form-control-sm" disabled value="">
                                <label _ngcontent-sdd-c96 for=razonSocial>Razón Social</label>
                            </div>
                        </div>
                    </div>
                    <div _ngcontent-sdd-c96 class=col-sm-2>
                        <div _ngcontent-sdd-c96 class=form-group>
                            <div _ngcontent-sdd-c96 class="form-floating form-floating-sm">
                                <select _ngcontent-sdd-c96 id=bl formcontrolname=indicadorBienLigitioso required
                                    class="form-select form-select-sm">
                                    <option _ngcontent-sdd-c96 value=true>Si</option>
                                    <option _ngcontent-sdd-c96 value=false selected>No</option>
                                </select>
                                <label _ngcontent-sdd-c96 for=bl>Bien Litigioso</label>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div _ngcontent-sdd-c96 class="row py-3">
                    <div _ngcontent-sdd-c96 class=col-sm-2>
                        <div _ngcontent-sdd-c96 class=form-group>
                    ...
                    <div _ngcontent-sdd-c96 class=col-sm-10>
                        <div _ngcontent-sdd-c96 class=form-group>
                            <div _ngcontent-sdd-c96 class="form-floating sm-4">
                                <textarea _ngcontent-sdd-c96 id=sc placeholder=# formcontrolname=descripcion
                                    maxlength=4999 required
                                    class="form-control form-control-sm"></textarea>
                                <label _ngcontent-sdd-c96 for=sc>Descripción</label>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div _ngcontent-sdd-c96 class=row>
                    <div _ngcontent-sdd-c96 class=col-sm-6>&nbsp;</div>
                    <div _ngcontent-sdd-c96 class=col-sm-6>
                        <div _ngcontent-sdd-c96 class=form-group>
                            <div _ngcontent-sdd-c96 class="form-floating sm-4">
                ...
                <button _ngcontent-sdd-c96 type=submit class="btn btn-sm btn-danger" disabled>Guardar <i _ngcontent-sdd-c96 class=bi-save></i></button>
            </form>
        </div>
    </div>
    <br _ngcontent-sdd-c96>

    <!-- ═══ Table ═══ -->
    <div id="tablaContainerCajaAhorro" style="display:none">
        <table _ngcontent-sdd-c96 class="table table-bordered table-striped table-sm">
            <thead _ngcontent-sdd-c96>
                <tr _ngcontent-sdd-c96>
                    <th _ngcontent-sdd-c96 scope=col>Tipo de Bien</th>
                    <th _ngcontent-sdd-c96 scope=col>Bien Litigioso</th>
                    <th _ngcontent-sdd-c96 scope=col>Descripción</th>
                    <th _ngcontent-sdd-c96 scope=col>Valor Declarado (Bs.)</th>
                    <th _ngcontent-sdd-c96 scope=col>Acción</th>
                </tr>
            </thead>
            <tbody _ngcontent-sdd-c96 id="tbodyCajaAhorro"></tbody>
        </table>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const tbody = document.getElementById('tbodyCajaAhorro');
        if (!form || !btn) return;
        ...
        function renderTable() {
            const container = document.getElementById('tablaContainerCajaAhorro');
            tbody.innerHTML = '';
            let totalDeclarado = 0;

            if (cajaAhorroItems.length === 0) {
                container.style.display = 'none';
            } else {
                container.style.display = '';
            }

            cajaAhorroItems.forEach(function (item, idx) {
                const vd = parseFloat((item.valor_declarado || '0').replace(/\./g, '').replace(',', '.'));
                totalDeclarado += vd;

                const desc = ` ${item.porcentaje || '0,01'}% de ${item.descripcion || ''}. Nombre de la Empresa: ${item.razon_social || ''}`;

                const tr = document.createElement('tr');
                tr.setAttribute('_ngcontent-sdd-c96', '');
                tr.innerHTML = `
                <td _ngcontent-sdd-c96>Caja de Ahorro</td>
                <td _ngcontent-sdd-c96>${item.bien_litigioso === 'true' ? 'Si' : 'No'}</td>
                <td _ngcontent-sdd-c96 class=lthgf><div _ngcontent-sdd-c96 style=width:auto> ${desc}</div></td>
                <td _ngcontent-sdd-c96 align=right>${item.valor_declarado || '0,00'}</td>
                <td _ngcontent-sdd-c96>
                    <div _ngcontent-sdd-c96 class=accionesicono>
                        <i _ngcontent-sdd-c96 class="bi bi-pencil-fill" onclick="editarCajaAhorro(${idx})" title="Modificar"></i>&nbsp;
                        <i _ngcontent-sdd-c96 class="bi-trash-fill" onclick="eliminarCajaAhorro(${idx})" title="Eliminar"></i>
                    </div>
                </td>`;
