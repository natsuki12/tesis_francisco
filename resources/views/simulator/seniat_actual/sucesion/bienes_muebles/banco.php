...
let bancos = <?= json_encode($bancosGuardados, JSON_UNESCAPED_UNICODE) ?>;
let editIndex = null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.editarBanco = function(idx) {
        const item = bancos[idx];
        if (!item) return;
        editIndex = idx;
    ...
    // ═══ Delete ═══
    window.eliminarBanco = function(idx) {
        if (!confirm('¿Está seguro de eliminar este registro?')) return;
        if (!INTENTO_ID) { alert('No hay intento activo'); return; }

        fetch(BASE + '/api/banco/' + INTENTO_ID + '/eliminar', {
            method: 'POST',
    ...
    form.addEventListener('submit', function(e) {
        ...
        const url = isEdit
            ? BASE + '/api/banco/' + INTENTO_ID + '/editar'
            : BASE + '/api/banco/' + INTENTO_ID + '/agregar';
        ...
        .then(data => {
            if (data.ok) {
                if (isEdit) {
                    bancos[editIndex] = formData;
                } else {
                    bancos.push(formData);
                }
                renderTable();
                resetForm();
            } else {
