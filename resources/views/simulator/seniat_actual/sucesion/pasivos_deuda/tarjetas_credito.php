...
<script>
    ...
    let tdcItems = <?= json_encode($tdcGuardados, JSON_UNESCAPED_UNICODE) ?>;
    let editIndex = null;
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.editarTdc = function (idx) {
            const item = tdcItems[idx];
            if (!item) return;
            editIndex = idx;
        ...
        // ═══ Delete ═══
        window.eliminarTdc = function (idx) {
            if (!confirm('¿Está seguro de eliminar este registro?')) return;
            if (!INTENTO_ID) { alert('No hay intento activo'); return; }

            fetch(BASE + '/api/tarjetas_credito/' + INTENTO_ID + '/eliminar', {
                method: 'POST',
        ...
        form.addEventListener('submit', function (e) {
            ...
            const url = isEdit
                ? BASE + '/api/tarjetas_credito/' + INTENTO_ID + '/editar'
                : BASE + '/api/tarjetas_credito/' + INTENTO_ID + '/agregar';
            ...
            fetch(url, {
                ...
                .then(data => {
                    if (data.ok) {
                        if (isEdit) {
                            tdcItems[editIndex] = formData;
                        } else {
                            tdcItems.push(formData);
                        }
                        renderTable();
                        resetForm();
                    } else {
